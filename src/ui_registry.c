...
bool ui_register_renderer(const ui_renderer_t *r) {
    if (!r || !r->name) {
        LOGW("null renderer or name");
        return false;
    }

    for (size_t i = 0; i < g_renderer_count; ++i) {
        if (strcmp(g_registered_renderers[i]->name, r->name) == 0) {
            LOGW("renderer '%s' already registered", r->name);
            return false; /* duplicate */
        }
    }

    if (g_renderer_count >= UI_MAX_RENDERERS) {
        LOGE("registry full (max=%d), cannot register '%s'", UI_MAX_RENDERERS, r->name);
        return false;
    }

    g_registered_renderers[g_renderer_count++] = r;
    LOGI("registered renderer '%s' (count=%zu)", r->name, g_renderer_count);
    return true;
}
...
const ui_renderer_t *ui_get_renderer(const char *name) {
    ...
    LOGW("renderer '%s' not found", name);
    return NULL;
}

const ui_renderer_t *ui_get_default_renderer(void) {
    if (g_renderer_count == 0) {
        LOGW("no renderers registered");
        return NULL;
    }
    LOGD("returning '%s'", g_registered_renderers[0]->name);
